...
### B. 詳細データ追加（parks.json）

特定の公園に住所・面積・トイレ情報などを付ける場合は `parks.json` に追加します。  
※現状は3件のサンプルのみで、本格運用は今後検討。
...
⚠️ **新規デプロイにすると URL が変わるため必ず既存デプロイの編集にすること**
...
├── parks-data.js          # 公園データ（PARKFUL調査 231件）
├── parks.json             # 詳細データ（サンプル3件）
